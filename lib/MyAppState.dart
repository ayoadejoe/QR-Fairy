...
class MyAppState extends ChangeNotifier {
  ...
  Future<void> saveNewUser(String username, String password, String email,
      String urls, BuildContext context) async {
    print('entering new user');
    isLoading = true;
    try {
      registerSuccess =
          await saveNewUserRestCall(username, password, email, urls);
      isLoading = true;
      showToast(
          context,
          'You are now registered. All qrcodes you generate would be saved',
          "REGISTERED");
      notifyListeners();
    } catch (e) {
      isLoading = false;
  ...
  Future<void> loginUser(String username, String password, String urls,
      BuildContext context) async {
    print('entering new user');
    isLoading = true;
    try {
      registerSuccess = await loginRestCall(username, password, urls);
      isLoading = true;
      showToast(context, 'Hi: All qrcodes you generate would now be saved',
          "WELCOME");
      notifyListeners();
    } catch (e) {
      isLoading = false;
